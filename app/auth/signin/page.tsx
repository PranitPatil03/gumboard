  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.data) {
        ...
        // Still show success state for UX
        setIsSubmitted(true);
      }
    } catch (error) {
      console.error("Authentication error:", error);
      // Show success state even on error for UX
      setIsSubmitted(true);
    } finally {
      setIsLoading(false);
    }
        ...
        <Card className="w-full max-w-sm sm:max-w-md bg-white/95 dark:bg-zinc-900/95 border border-gray-200 dark:border-zinc-800 shadow-sm hover:shadow-md transition-shadow">
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground text-center mb-4 dark:text-zinc-400">
              Click the link in the email to sign in to your account. The link will expire in 5
              minutes.
            </p>
